...
| feat-51-politica-releases-branches | feat/51-politica-releases-branches | done |

## Próximo passo
**Iniciar:** Nenhuma feature `todo` pendente. feat-51 concluída na branch `feat/51-politica-releases-branches` — push + PR pendentes. Pacote feat-02 (7/7 features) segue completo e pronto para publicação — push + PR pendentes; v2.0.0-beta.0 "Forge-SDD Slim" também segue aguardando decisão do usuário sobre promover a estável.
**Bloqueios:** —

## Handoff da última sessão
- feat-51-politica-releases-branches concluída: critério de "3 últimas releases" confirmado com o usuário como "3 estáveis mais recentes + qualquer beta publicada depois da mais antiga dessas 3". Implementado `prerelease: auto` em `.goreleaser.yaml`; novo step "Enforce Beta Release Highlights" em `.github/workflows/release.yml` que falha o workflow se uma tag beta cair no texto de fallback genérico das release notes; novo step "Enforce Release Retention Policy" que roda após o GoReleaser e apaga releases/tags/rascunhos fora da política a cada nova publicação (usa `gh api` porque `gh release delete` não resolve rascunhos duplicados com a mesma tag); nova frase na Regra 12 da Constituição documentando a política (NPM não é afetado — pacotes já publicados são imutáveis). Limpeza pontual executada nesta sessão com confirmação explícita do usuário: 20 releases antigas + 21 rascunhos órfãos apagados no GitHub (restam `v2.2.0-beta`, `v2.0.0-beta`, `v1.9.4`, `v1.9.2`, `v1.9.1`), as duas betas retroativamente marcadas `prerelease=true` via API, e 13 branches remotas com PR já mergeado removidas. 3 branches em zona cinzenta (`feat/npm-release-and-deploy`, `feat/open-source-readme`, `feat/release-44-fluxo-sdd` — sem PR merged, com commits fora de `main`) foram preservadas a pedido do usuário. Ver `sdd/features/feat-51-politica-releases-branches.md`.
