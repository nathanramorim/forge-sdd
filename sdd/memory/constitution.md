...
15. Nova Convenção de Versionamento: O formato das tags de versão segue a estrutura de `1.<feature>.<fix>`. Versões de teste/homologação beta utilizam o sufixo simples `-beta` (ex: `1.8.0-beta`), sem sufixos numéricos adicionais, acumulando alterações locais antes do lançamento da versão estável correspondente (ex: `1.8.0`).
