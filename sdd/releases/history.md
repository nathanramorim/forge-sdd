...
### Versão 1.8.1-beta (Beta)

Esta release beta adiciona proteção de customizações locais de agentes no update/upgrade.

*   **Backup Automático no Upgrade:** O CLI realiza cópias automáticas de segurança (backups) em `sdd/backups/upgrade-<timestamp>/` antes de sobrescrever arquivos de prompts ou instruções de qualquer agente (Copilot, Gemini, Claude, OpenAI).
