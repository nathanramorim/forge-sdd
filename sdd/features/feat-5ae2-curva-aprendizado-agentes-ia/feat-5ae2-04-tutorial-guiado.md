...
Implementado o prompt `/tutorial` nos três agentes (Claude, Gemini, Copilot), inserido na ordem pedagógica do cheat-sheet logo após `/constitution`. A flag `--tutorial` do `forge-sdd init` apenas imprime a dica de rodar `/tutorial` ao final do scaffold (fora de `--dry-run`) — a criação dos artefatos de exemplo isolados (`sdd/discovery/_tutorial/`, `sdd/features/_tutorial/`) e o guardrail de não tocar `index.md`/`progress.md`/`.metrics/` ficam a cargo do próprio prompt de chat. Testes em `cmd/forge-sdd/commands_test.go` (`TestInitCommand_TutorialFlagPrintsHint`, `TestInitCommand_TutorialFlagSkippedOnDryRun`).
