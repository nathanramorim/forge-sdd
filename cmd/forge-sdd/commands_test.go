...
func TestDoctorAndDestroyCommands(t *testing.T) {
	...
	err = os.WriteFile(filepath.Join(sddDir, ".sdd-version"), []byte("1.9.2"), 0644)
	require.NoError(t, err)
...
func TestDoctorCommand_NamingConventionConsistent(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(sddDir, ".sddrc"), []byte(`{"project":"test-proj","version":"1.9.2","agents":[],"stack":"go","db":"none","lang":"pt-BR"}`), 0644))
