...
func TestDryRunNoFiles(t *testing.T) {
	...
	cfg := config.Config{
		...
		Telemetry: false, Lang: "pt-BR", SddVersion: "1.7.1-beta.4",
...
func TestUpgradePreservesDomain(t *testing.T) {
	...
	// 2. Segunda execução (simula upgrade para v1.7.1-beta.4 com novos templates)
	cfgUpgrade := config.Config{
		...
		Telemetry:  false,
		Lang:       "pt-BR",
		SddVersion: "1.7.1-beta.4", // nova versão
		Agents:     []string{config.AgentGemini},
	}
	...
	// b. .sdd-version DEVE ter sido atualizado para 1.7.1-beta.4
	versionData, err := os.ReadFile(versionFile)
	require.NoError(t, err)
	assert.Contains(t, string(versionData), "1.7.1-beta.4")
