...
func TestDryRunNoFiles(t *testing.T) {
	...
	cfg := config.Config{
		...
		Telemetry: false, Lang: "pt-BR", SddVersion: "1.9.2",
...
func TestUpgradePreservesDomain(t *testing.T) {
	...
	// 2. Segunda execução (simula upgrade para v1.9.2 com novos templates)
	...
	// b. .sdd-version DEVE ter sido atualizado para 1.9.2
	versionData, err := os.ReadFile(versionFile)
	require.NoError(t, err)
	assert.Contains(t, string(versionData), "1.9.2")
