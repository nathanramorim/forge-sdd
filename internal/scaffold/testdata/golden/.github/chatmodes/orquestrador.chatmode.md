...
10. **PR & Merge (gh CLI):** Suba as alterações com `git push origin <branch>`. Prefira usar a CLI `gh` para criar e mesclar o Pull Request:
    - Criar PR: `gh pr create --fill`
    - Mesclar PR: `gh pr merge --squash --delete-branch`
    - Se o `gh` não estiver disponível, caia para o merge local na main: `git checkout main && git merge --no-ff <branch>`
